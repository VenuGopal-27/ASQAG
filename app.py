...
app.config.update(
    ...
    MAIL_PASSWORD='hvag cdvc anxr nswq'
)
mail = Mail(app)
...
def process_form():
    ...
    def generate_questions(text, level):
        ...
        # Extracting best root word
        lemmatizer = WordNetLemmatizer()
        lemmatized_words = [lemmatizer.lemmatize(word) for word in filtered_words]

        # Assigning pos_tagging
        pos_tags = pos_tag(lemmatized_words)

        questions = []
        count = 0
        # Mapping bloom's levels
        count1 = int(request.form['question-count'])
        for word, word_pos in pos_tags:
            ...
            if word.isalnum():
                if level == 1:
                    # Questions for Bloom's level 1 for each POS tag
                    ...
                    # Questions for Bloom's level 2 for each POS tag
                    if word_pos.startswith('CC'):  # Coordinating Conjunction
                        questions.append(
                    ...
                    # Questions for Bloom's level 3 for each POS tag
                    if word_pos.startswith('CC'):  # Coordinating Conjunction
                        questions.append(
                    ...
                    # Questions for Bloom's level 4 for each POS tag
                    if word_pos.startswith('CC'):  # Coordinating Conjunction
                        questions.append(
                    ...
                    # Questions for Bloom's level 5 for each POS tag
                    if word_pos.startswith('CC'):  # Coordinating Conjunction
                        questions.append(
                    ...
                    # Questions for Bloom's level 6 for each POS tag
                    if word_pos.startswith('CC'):  # Coordinating Conjunction
                        questions.append(
...
def process_answers():
    ...
    def calculate_similarity(answers, user_answers):
        ...
        nlp = en_core_web_lg.load()  # Load the larger spaCy model with word vectors
